...
/*
 * Help Links
 * - https://github.com/searchkit/searchkit/blob/main/packages/searchkit/src/Transporter.ts
 *
 */

const createElasticsearchQueryFromRequest = (requests: SearchRequest[]) => {
  return requests
    .reduce<string[]>(
      (sum, request) => [
        ...sum,
...
export class ESTransporter implements Transporter {
  ...
  async performNetworkRequest(requests: SearchRequest[]) {
    ...
    const host = this.config.cloud_id
      ? getHostFromCloud(this.config.cloud_id)
      : this.config.host;
  ...
  async msearch(
    ...
    try {
      ...
      if (responses.status >= 500) {
        ...
        responses.responses?.[0]?.status === 404
      ) {
        console.error(JSON.stringify(responses));
        throw new Error(
        ...
        responses.responses?.[0]?.status === 400
      ) {
        console.error(JSON.stringify(responses));
        throw new Error(
